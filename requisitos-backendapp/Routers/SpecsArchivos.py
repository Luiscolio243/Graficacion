...
def gen_00_indice(proyecto, archivos_presentes):
    ...
    L.append("")
    L.append("> La IA debe generar estos scripts y documentarlos explícitamente en el `README.md`.")
    L.append("> El usuario debe ejecutar el mínimo de comandos posibles.")
    L.append("")
    L.append("### Base de datos")
    L.append("```bash")
    L.append("# 1. Crear la base de datos en PostgreSQL")
    L.append("createdb nombre_bd")
    L.append("# O desde psql:")
    L.append("# CREATE DATABASE nombre_bd;")
    L.append("```")
    L.append("")
    L.append("### Backend — UN solo comando")
    L.append("```bash")
    L.append("# Windows:")
    L.append("setup.bat")
    L.append("# Linux/Mac:")
    ...
    L.append("```")
    L.append("")
    L.append("### El script setup hace automáticamente:")
    L.append("```")
    L.append("1. Crear entorno virtual")
    L.append("2. pip install -r requirements.txt --only-binary=:all:")
    L.append("3. Copiar .env.example a .env")
    L.append("4. flask db upgrade")
    L.append("5. python seed_db.py")
    L.append("6. python run.py")
    L.append("```")
    L.append("")
    L.append("### Frontend — UN solo comando")
    L.append("```bash")
    L.append("npm install && npm run dev")
    L.append("```")
    L.append("")
    L.append("### Usuarios de prueba generados por seed_db.py")
    L.append("La IA debe imprimir en consola los usuarios y contraseñas de prueba al correr el seed.")
    return "\n".join(L)
...
def gen_06_backend(tecnicas, session=None, id_proyecto=None):
    ...
    L.append("")
    L.append("- Usar `psycopg2-binary>=2.9.9` en `requirements.txt` — **nunca versión exacta**")
    L.append("- Instalar con: `pip install psycopg2-binary --only-binary=:all:`")
    L.append("- Si falla la instalación: reemplazar por `pg8000` que no requiere compilación")
    L.append("- **IMPORTANTE:** Si el proyecto o la BD tiene nombres con acentos o caracteres especiales,")
    L.append("  usar `pg8000` en lugar de `psycopg2-binary` para evitar errores de encoding")
    L.append("- La cadena de conexión debe incluir `client_encoding=utf8` si se usa psycopg2")
    L.append("- Incluir instrucciones de instalación en el `README.md` del proyecto")
    L.append("")
    L.append("## Script de instalación automática")
    L.append("")
    L.append("Genera un script `setup.sh` (Linux/Mac) y `setup.bat` (Windows) que ejecute")
    L.append("automáticamente todos los pasos de instalación sin intervención manual:")
    L.append("")
    L.append("**El script debe hacer en orden:**")
    L.append("1. Crear y activar el entorno virtual")
    L.append("2. Instalar todas las dependencias con `pip install -r requirements.txt --only-binary=:all:`")
    L.append("3. Crear el archivo `.env` desde `.env.example` si no existe")
    L.append("4. Correr las migraciones: `flask db upgrade`")
    L.append("5. Correr el seed: `python seed_db.py`")
    L.append("6. Levantar el servidor: `python run.py`")
    L.append("")
    L.append("**El usuario solo debe ejecutar UN comando:**")
    L.append("- Windows: `setup.bat`")
    L.append("- Linux/Mac: `bash setup.sh`")
    L.append("")
