...
export default function Entrevistas(){
  ...
  useEffect(() => {
    const obtenerEntrevistas = async () => {
      try {
        const response = await fetch(`http://localhost:5000/entrevistas/${id}`);
  ...
  const eliminarEntrevista = (id_entrevista) => {
    setEntrevistas(entrevistas.filter((e) => e.id_entrevista !== id_entrevista));
  };
